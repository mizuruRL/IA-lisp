...
(defun get-input-line ()
    ...
    (let ((line (read)))
        (cond ((null line) (error-handle 'get-input-line "Error: No input.~%"))
            ...
            ((or (< 6 line) (> 0 line)) (error-handle 'get-input-line "Error: Line out of board range.~%"))
            (t line)
        )
    )
)
...
(defun get-input-column ()
    ...
    (let ((column (read)))
        (cond ((null column) (error-handle 'get-input-column "Error: No input.~%"))
            ...
            ((or  (< 7 column) (> 0 column)) (error-handle 'get-input-column "Error: Column out of board range.~%"))
            (t column)
        )
    )
)
