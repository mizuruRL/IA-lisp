...
(defun check-all-closed-boxes (board &optional (line 1) (column 1) (player 1))
"Checks how many closed boxes a given BOARD has."
    (cond ((not (in-bounds-horizontal (car board) (1+ line) 1)) 0)
          ((not (in-bounds-horizontal (car board) 1 column)) (check-all-closed-boxes board (1+ line) 1 player))
          ((check-box board line column player) (1+ (check-all-closed-boxes board line (1+ column) player)))
          (t (check-all-closed-boxes board line (1+ column) player))
    )
)
...
(defun play-piece (player board line column arc-fun)
    (funcall arc-fun board line column player)
)